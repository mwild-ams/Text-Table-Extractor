...
figma.ui.onmessage = (msg) => {
  ...
  if (msg.type === 'scan-document') {
    if (msg.fixString.length <= 0) {
        // TODO: Handle this in a more notifying way
        figma.notify("Please enter a prefix/suffix!");
        return;
    }
    ...
    for (let n of textNodes) {
        ...
        if (msg.prefixBool) {
            if (n.name.startsWith(msg.fixString)) {
                contentIDPairs.push([n.name.substring(msg.fixString.length), n.characters]);
            }
        }
        else {
            if (n.name.endsWith(msg.fixString)) {
                contentIDPairs.push([n.name.substring(0, n.name.length - msg.fixString.length), n.characters]);
            }
        }
    }
    ...
    while (index < contentIDPairs.length - 1) {
        if (contentIDPairs[index][0].localeCompare(contentIDPairs[index + 1][0]) == 0 &&
            contentIDPairs[index][1].localeCompare(contentIDPairs[index + 1][1]) == 0) {
    ...
    while (i < contentIDPairs.length - 1) {
        ...
        while (n < contentIDPairs.length - 1) {
            ...
            if (contentIDPairs[n][0].localeCompare(contentIDPairs[n + 1][0]) == 0 ||
                ((n > 0) ? contentIDPairs[n][0].localeCompare(contentIDPairs[n - 1][0]) == 0 : false)) {
                conflictIDSet.push(n);
            }
            // In case the last two are part of a set
            if (n == contentIDPairs.length - 2 && contentIDPairs[n][0].localeCompare(contentIDPairs[n + 1][0]) == 0) {
        ...
        if (conflictIDSet.length > 0)
            conflictIDIndices.push(conflictIDSet);
    }
    console.log(contentIDPairs);
    console.log(conflictIDIndices);
    figma.ui.postMessage({ type: "scan-results", contentIDPairs, conflictIDIndices });
  }
